...
svg2js(testSvg, function(svgjs) {
  ...
  console.log(testSvg.length, svg.data.length);

  var svgz = gzip(svg.data);
  console.log(svgz.length);

  var a = document.createElement('a');
  a.textContent = 'download';
  a.download = 'output.svgz';
  a.href = URL.createObjectURL(new Blob([svgz], {type: "image/svg+xml"}));
