...
async function compress(svgStr) {
  var originalSize = gzip.compress(svgStr);
  var svgoResult = await svgo.process(svgStr);

  svgOuput.setSvg(svgoResult.data);
  codeOutput.setCode(svgoResult.data);
  downloadButton.setDownload("output.svg", svgoResult.data);

  var svgz = gzip.compress(svgoResult.data);

  results.update(
    (await originalSize).byteLength,
    (await svgz).byteLength
  );
}
